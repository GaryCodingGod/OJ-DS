...
public:
	...
	LinkList<GraphNode<T> *> *getList() const
	{
		return list;
	}
private:
...
public:
	...
	bool isForest()
	{
		...
		// 使用DFS來檢查是否有環
		auto hasCycle = [&](GraphNode<T> *node, GraphNode<T> *parent, auto &dfs) -> bool {
			ListNode<GraphNode<T> *> *neighbor = node->getList()->getHead();
			visited[node->getData() - 'A'] = true;

			while (neighbor != NULL) {
				GraphNode<T> *adj = neighbor->getData();

				// 如果節點尚未訪問，繼續遞迴
				if (!visited[adj->getData() - 'A']) {
					if (dfs(adj, node, dfs))
						return true;
				} else if (adj != parent) {
					return true;
		...
		// 檢查每個連通子圖，是否都無環且已完全遍歷
		for (int i = 0; i < count; i++) {
			GraphNode<T> *current = (*vertex)[i].getData();
			if (!visited[current->getData() - 'A']) {
				if (hasCycle(current, NULL, hasCycle)) {
					return false; 
				}
			}
		}
