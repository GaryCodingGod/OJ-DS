...
public:
	...
	ListNode<T> *removeFromHead()
	{
		...
		if(head != NULL)
		{
			...
			if(head != NULL)
				...
				tail = NULL; // 如果移除後鏈表為空，設置尾節點為 NULL
			n->setNext(NULL);
		}
		return n;
	}
	ListNode<T> *removeFromTail()
	{
		...
		if(tail != NULL)
		{
			tail = tail->getPrev(); // 更新尾節點
			if(tail != NULL)
				tail->setNext(NULL);
			else
				head = NULL; // 如果移除後鏈表為空，設置頭節點為 NULL
			n->setPrev(NULL);
		}
		return n;
	}
...
public:
	...
	WeightedGraphVertex<V, E> *&getAnotherEnd(WeightedGraphVertex<V, E> *v) //訪問另一端頂點的方法
	{
		if(v != end[0])
			return end[0];
		return end[1];
...
public:
	...
	void BFS(WeightedGraphVertex<V, E> *v)
	{
		...
		set<WeightedGraphVertex<V, E> *> visited; //set的主要是用來追蹤和記錄已經訪問過的節點，防止在遍歷過程中重複訪問
		q.push(v);
		visited.insert(v);//表示該節點已訪問
		while(!q.empty())
		{
			WeightedGraphVertex<V, E> *current = q.front();
			q.pop();
			cout << current->getData() << " ";
			for (int i = 0; i < current->list->getSize(); i++)
			{
				WeightedGraphEdge<V, E> *edge = (*current->list)[i].getData();//取出邊
				WeightedGraphVertex<V, E> *neighbor = edge->getAnotherEnd(current);//獲取相鄰節點
				if (visited.find(neighbor) == visited.end())  //如果相鄰節點未被訪問
				{
					q.push(neighbor);
	...
	void DFS(WeightedGraphVertex<V, E> *v)
	{
		...
		while(!s.empty())
		{
			WeightedGraphVertex<V, E> *current = s.top(); // 取出堆疊的頂部節點
			s.pop();
			if (visited.find(current) == visited.end()) //如果節點未被訪問
			{
				cout << current->getData() << " ";
